"""UMAP visualization: Teacher vs v3 vs v4 on 16 languages × 50 parallel sentences."""
import os
import sys
import numpy as np
...
STUDENT_V4 = "C:/Users/Moon/finetuning-workshop/SmallModel/students/me5s/me5s_compressed_v4_distilled"
...
emb_v3 = embed(STUDENT_V3, "v3_distilled")
emb_v4 = embed(STUDENT_V4, "v4_distilled (ep4)")

models = [
    (emb_teacher, "Teacher (me5s-small)\n384d, ~448MB"),
    (emb_v3,      "v3_distilled (byte_fallback)\n384d, ~53MB"),
    (emb_v4,      "v4_distilled (anchor +5.6K)\n384d, ~59MB"),
]

print("\nRunning UMAP on each...")
xy_list = []
for emb, _ in models:
    reducer = umap.UMAP(n_components=2, n_neighbors=15, min_dist=0.1,
                        metric="cosine", random_state=42)
    xy_list.append(reducer.fit_transform(emb))
...
for row, color_by in enumerate(["lang", "sent_id"]):
    for col, ((emb, title), xy) in enumerate(zip(models, xy_list)):
        ax = axes[row, col]
        if color_by == "lang":
...
fig.suptitle(f"UMAP: {len(LANGS)} languages × {n} parallel sentences (MASSIVE) — Teacher vs v3 vs v4",
             fontsize=13)
plt.tight_layout()
out = "C:/Users/Moon/finetuning-workshop/SmallModel/umap_me5s_v4.png"
