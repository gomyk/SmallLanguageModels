...
/**
 * UnigramTokenizer Android Instrumented Test
 *
 * 210개 TC 중 Unigram 105개를 실제 단말에서 검증한다.
 * golden_unigram.json (HuggingFace 기준 expected IDs)을 assets에서 로드하여
 * UnigramTokenizer.encode() 결과와 1:1 비교.
 *
 * Setup:
 *   1. tokenizer.json → app/src/androidTest/assets/unigram_tokenizer.json
 *   2. golden_unigram.json → app/src/androidTest/assets/golden_unigram.json
 *
 * Run:
 *   ./gradlew connectedAndroidTest --tests "*.UnigramTokenizerTest"
 */
@RunWith(AndroidJUnit4::class)
class UnigramTokenizerTest {

    private lateinit var tokenizer: UnigramTokenizer
    private lateinit var golden: Map<String, Pair<String, List<Int>>>

    @Before
    fun setUp() {
        val ctx = InstrumentationRegistry.getInstrumentation().targetContext

        tokenizer = ctx.assets.open("unigram_tokenizer.json").use {
            UnigramTokenizer.fromInputStream(it)
        }

        golden = loadGolden(ctx, "golden_unigram.json")
    }
    ...
    private fun assertEncode(category: String) {
        ...
        if (failures.isNotEmpty()) {
            fail("Unigram [$category] ${failures.size}/$count FAILED:\n${failures.joinToString("\n")}")
        }
    }
    ...
    @Test
    fun clsSepTokenPosition() {
        val texts = listOf("Hello", "안녕하세요", "Test 123")
        val clsId = 0   // <s>
        val sepId = 2   // </s>
        for (text in texts) {
            val ids = tokenizer.encode(text).inputIds
            assertTrue("Should have at least 3 tokens (CLS + content + SEP)", ids.size >= 3)
            assertEquals("First token should be CLS for: $text", clsId, ids.first())
            assertEquals("Last token should be SEP for: $text", sepId, ids.last())
        }
    }

    // ═══════════════════════════════════════════════════════════
    // 빈 문자열 → [CLS, SEP] 만 나와야 함
    // ═══════════════════════════════════════════════════════════

    @Test
    fun emptyStringMinimal() {
        val ids = tokenizer.encode("").inputIds
        assertEquals("Empty string should produce [CLS, SEP]", 2, ids.size)
        assertEquals(0, ids[0])  // CLS
        assertEquals(2, ids[1])  // SEP
    }
    ...
    // ═══════════════════════════════════════════════════════════
    // NFKC 정규화 검증: fullwidth → halfwidth 변환
    // ═══════════════════════════════════════════════════════════

    @Test
    fun nfkcNormalization() {
        // fullwidth "ＨＥＬＬＯ" → NFKC → "HELLO" → 같은 토큰
        val fullwidth = tokenizer.encode("ＨＥＬＬＯ").inputIds.toList()
        val halfwidth = tokenizer.encode("HELLO").inputIds.toList()
        assertEquals("NFKC: fullwidth should normalize to halfwidth", halfwidth, fullwidth)
    }

    // ═══════════════════════════════════════════════════════════
    // Hangul Compatibility Jamo 보존 검증
    // ═══════════════════════════════════════════════════════════

    @Test
    fun hangulJamoPreservation() {
        // ㄱ (U+3131) 은 NFKC에서 ᄀ (U+1100)으로 바뀌면 안 됨
        val ids = tokenizer.encode("ㄱㄴㄷ").inputIds
        assertTrue("Hangul jamo should produce tokens", ids.size > 2) // CLS + content + SEP
    }
    ...
    private fun assertSingle(key: String) {
        ...
        val actual = tokenizer.encode(text).inputIds.toList()
        assertEquals(
            "Unigram mismatch [$key]: text=${text.take(40)}",
            expectedIds, actual
        )
    }
}
